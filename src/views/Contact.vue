...
function updatePhoneStatus(index: number, status: boolean) {
  if (currentContact.value)
    currentContact.value.phones[index].wrong = status
}
      ...
      <div v-if="showModal" class="fixed top-0 right-0 bottom-0 z-50 overflow-auto custom-inner-background">
        <div class="relative bg-white p-8 w-full sm:w-auto h-full flex flex-col rounded-l-lg shadow-lg custom-inner-background">
          ...
          <div>
            ...
            <div v-for="(phone, index) in currentContactPhones" :key="index" class="flex items-center mt-2">
              <input
              ...
              <button
                ...
                :class="{ 'text-green-500': phone.wrong === false }"
                @click="updatePhoneStatus(index, false)"
              >
              ...
              <button
                class="focus:outline-none ml-2"
                :class="{ 'text-red-500': phone.wrong === true }"
                @click="updatePhoneStatus(index, true)"
              >
                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
